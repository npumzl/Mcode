...
class SerialAssistant(tk.Tk):
    ...
    # 创建QApplication实例
        ...
        # 创建绘图窗口
        self.plot = pg.plot(title='动态更新曲线')
        self.data1plot = self.plot.plot(pen='r', name='Data1')  # 绘制a的曲线
        self.data2plot = self.plot.plot(pen='g', name='Data2')  # 绘制a的曲线
        self.data3plot = self.plot.plot(pen='b', name='Data3')  # 绘制a的曲线
        self.data4plot = self.plot.plot(pen='c', name='Data4')  # 绘制a的曲线
        self.data5plot = self.plot.plot(pen='m', name='Data5')  # 绘制a的曲线
        self.data6plot = self.plot.plot(pen='y', name='Data6')  # 绘制a的曲线
        ...
        if self.ser:
            ...
            if self.data:
               ...
               # data_list为一个包含时间+6data的列表
                ...
                self.update_plot()
                # # 每100次将数据放到队列中
                # if len(self.databuffer) == 100:#100个 6*1的数据
                #     self.data_queue.put(list(self.databuffer))
                #     if self.execute_flag.get():# 判断是否勾选绘图功能
                #         pass
                #         #self.update_plot()
    ...
    def update_plot(self):
        if self.ser and self.data:
            try:
                ...
                # self.data1plot.setData(self.data1_list[-300:])
                # self.data2plot.setData(self.data2_list[-300:])
                # self.data3plot.setData(self.data3_list[-300:])
                self.data1plot.setData(self.data1_list)
                self.data2plot.setData(self.data2_list)
                self.data3plot.setData(self.data3_list)
                self.data4plot.setData(self.data4_list)
                self.data5plot.setData(self.data5_list)
                self.data6plot.setData(self.data6_list)

                #print(self.data1_list)
