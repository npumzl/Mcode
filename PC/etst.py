...
class SerialAssistant(tk.Tk):
    ...
    def receive_data(self):
        ...
        self.timer_receive = self.after(10, self.receive_data)  # 每20毫秒读取一次串口数据
    ...
    def update_current_time(self):
        ...
        self.timer_time = self.after(1000, self.update_current_time)  # 每隔1秒更新一次时间
    ...
    def create_state_frame(self):
        ...
        status_frame.grid(row=4, column=0, columnspan=2, sticky=tk.W  + tk.N, padx=10, pady=2)
        # 串口列表下拉菜单
        ...
        close_button.grid(row=0,column=1,padx=100,sticky="e")
    ...
    def close_program(self):
        if self.ser:
            self.ser.close()
        # 停止定时器
        self.after_cancel(self.timer_receive)
        self.after_cancel(self.timer_time)
        time.sleep(0.02)
        self.destroy()
        sys.exit()
