...
class SerialAssistant(tk.Tk):
    ...
    def create_serial_frame(self):
        ...
        self.baudrate_combobox = ttk.Combobox(serial_frame, textvariable=self.baudrate_var, width=10)
        self.baudrate_combobox.grid(row=1, column=1, sticky=tk.W)
        self.baudrate_combobox['values'] = [
            '2400',
            '4800',
            '9600',
            '14400',
    ...
    def create_command_frame(self):
        ...
        self.send_button1 = ttk.Button(command_frame, text="命令发送", command=self.send_command1, width=8)
        self.send_button1.grid(row=0, column=2)
        ...
        self.send_button2 = ttk.Button(command_frame, text="命令发送", command=self.send_command2, width=8)
        self.send_button2.grid(row=1, column=2)
    ...
    def receive_data(self):
        if self.ser:
            ...
            if self.data:
                ...
                # 用于计数有效数据个数
                val = int.from_bytes(self.data[1:3], byteorder='big', signed=False)
                self.ydata0.append(val)
                self.data_count.set(len(self.ydata0))

                # 将数据转换成列表格式，并添加到缓冲区中
                data_list = [time_str]
                for i in range(6):
                    data_list.append(int.from_bytes(self.data[(i * 2 + 1):(i * 2 + 3)], byteorder='big', signed=False))
    ...
    def update_plot(self):
        if self.ser and self.data:
            try:
                ...
                # 更新子图数据
                self.on_combobox1_change()
                self.on_combobox2_change()
                self.on_combobox3_change()
                self.on_combobox4_change()
            except serial.SerialException as e:
    ...
    def on_combobox1_change(self,event):
        ...
        if selected_option == 'None':
            ...
            # 在第1个子图中绘制收到的数据
            self.ax1.clear()
            self.ax1.plot(list(range(len(self.data1_list))), self.data1_list, 'r-', label='Data1')
            self.ax1.legend()
            self.ax1.set_title('Data1')
        elif selected_option == 'Data2':
            # 在第2个子图中绘制收到的数据
            self.ax1.clear()
            self.ax1.plot(list(range(len(self.data2_list))), self.data2_list, 'r-', label='Data2')
            self.ax1.legend()
            self.ax1.set_title('Data2')
        elif selected_option == 'Data3':
            # 在第3个子图中绘制收到的数据
            self.ax1.clear()
            self.ax1.plot(list(range(len(self.data3_list))), self.data3_list, 'r-', label='Data3')
            self.ax1.legend()
            self.ax1.set_title('Data3')
        elif selected_option == 'Data4':
            # 在第4个子图中绘制收到的数据
            self.ax1.clear()
            self.ax1.plot(list(range(len(self.data4_list))), self.data4_list, 'r-', label='Data4')
            self.ax1.legend()
            self.ax1.set_title('Data4')
        elif selected_option == 'Data5':
            # 在第5个子图中绘制收到的数据
            self.ax1.clear()
            self.ax1.plot(list(range(len(self.data5_list))), self.data5_list, 'r-', label='Data5')
            self.ax1.legend()
            self.ax1.set_title('Data5')
        elif selected_option == 'Data6':
            # 在第6个子图中绘制收到的数据
            self.ax1.clear()
            self.ax1.plot(list(range(len(self.data6_list))), self.data6_list, 'r-', label='Data6')
            self.ax1.legend()
            self.ax1.set_title('Data6')
        self.canvas.draw()
    ...
    def on_combobox2_change(self,event):
        ...
        if selected_option == 'None':
            ...
            # 在第1个子图中绘制收到的数据
            self.ax2.clear()
            self.ax2.plot(list(range(len(self.data1_list))), self.data1_list, 'r-', label='Data1')
            self.ax2.legend()
            self.ax2.set_title('Data1')
        elif selected_option == 'Data2':
            # 在第2个子图中绘制收到的数据
            self.ax2.clear()
            self.ax2.plot(list(range(len(self.data2_list))), self.data2_list, 'r-', label='Data2')
            self.ax2.legend()
            self.ax2.set_title('Data2')
        elif selected_option == 'Data3':
            # 在第3个子图中绘制收到的数据
            self.ax2.clear()
            self.ax2.plot(list(range(len(self.data3_list))), self.data3_list, 'r-', label='Data3')
            self.ax2.legend()
            self.ax2.set_title('Data3')
        elif selected_option == 'Data4':
            # 在第4个子图中绘制收到的数据
            self.ax2.clear()
            self.ax2.plot(list(range(len(self.data4_list))), self.data4_list, 'r-', label='Data4')
            self.ax2.legend()
            self.ax2.set_title('Data4')
        elif selected_option == 'Data5':
            # 在第5个子图中绘制收到的数据
            self.ax2.clear()
            self.ax2.plot(list(range(len(self.data5_list))), self.data5_list, 'r-', label='Data5')
            self.ax2.legend()
            self.ax2.set_title('Data5')
        elif selected_option == 'Data6':
            # 在第6个子图中绘制收到的数据
            self.ax2.clear()
            self.ax2.plot(list(range(len(self.data6_list))), self.data6_list, 'r-', label='Data6')
            self.ax2.legend()
            self.ax2.set_title('Data6')
        self.canvas.draw()

    def on_combobox3_change(self,event):
        selected_option = self.comboBox3.get()
        if selected_option == 'None':
            self.ax3.clear()
        elif selected_option == 'Data1':
            # 在第1个子图中绘制收到的数据
            self.ax3.clear()
            self.ax3.plot(list(range(len(self.data1_list))), self.data1_list, 'r-', label='Data1')
            self.ax3.legend()
            self.ax3.set_title('Data1')
        elif selected_option == 'Data2':
            # 在第2个子图中绘制收到的数据
            self.ax3.clear()
            self.ax3.plot(list(range(len(self.data2_list))), self.data2_list, 'r-', label='Data2')
            self.ax3.legend()
            self.ax3.set_title('Data2')
        elif selected_option == 'Data3':
            # 在第3个子图中绘制收到的数据
            self.ax3.clear()
            self.ax3.plot(list(range(len(self.data3_list))), self.data3_list, 'r-', label='Data3')
            self.ax3.legend()
            self.ax3.set_title('Data3')
        elif selected_option == 'Data4':
            # 在第4个子图中绘制收到的数据
            self.ax3.clear()
            self.ax3.plot(list(range(len(self.data4_list))), self.data4_list, 'r-', label='Data4')
            self.ax3.legend()
            self.ax3.set_title('Data4')
        elif selected_option == 'Data5':
            # 在第5个子图中绘制收到的数据
            self.ax3.clear()
            self.ax3.plot(list(range(len(self.data5_list))), self.data5_list, 'r-', label='Data5')
            self.ax3.legend()
            self.ax3.set_title('Data5')
        elif selected_option == 'Data6':
            # 在第6个子图中绘制收到的数据
            self.ax3.clear()
            self.ax3.plot(list(range(len(self.data6_list))), self.data6_list, 'r-', label='Data6')
            self.ax3.legend()
            self.ax3.set_title('Data6')
        self.canvas.draw()
    ...
    def on_combobox4_change(self,event):
        ...
        if selected_option == 'None':
            ...
            # 在第1个子图中绘制收到的数据
            self.ax4.clear()
            self.ax4.plot(list(range(len(self.data1_list))), self.data1_list, 'r-', label='Data1')
            self.ax4.legend()
            self.ax4.set_title('Data1')
        elif selected_option == 'Data2':
            # 在第2个子图中绘制收到的数据
            self.ax4.clear()
            self.ax4.plot(list(range(len(self.data2_list))), self.data2_list, 'r-', label='Data2')
            self.ax4.legend()
            self.ax4.set_title('Data2')
        elif selected_option == 'Data3':
            # 在第3个子图中绘制收到的数据
            self.ax4.clear()
            self.ax4.plot(list(range(len(self.data3_list))), self.data3_list, 'r-', label='Data3')
            self.ax4.legend()
            self.ax4.set_title('Data3')
        elif selected_option == 'Data4':
            # 在第4个子图中绘制收到的数据
            self.ax4.clear()
            self.ax4.plot(list(range(len(self.data4_list))), self.data4_list, 'r-', label='Data4')
            self.ax4.legend()
            self.ax4.set_title('Data4')
        elif selected_option == 'Data5':
            # 在第5个子图中绘制收到的数据
            self.ax4.clear()
            self.ax4.plot(list(range(len(self.data5_list))), self.data5_list, 'r-', label='Data5')
            self.ax4.legend()
            self.ax4.set_title('Data5')
        elif selected_option == 'Data6':
            # 在第6个子图中绘制收到的数据
            self.ax4.clear()
            self.ax4.plot(list(range(len(self.data6_list))), self.data6_list, 'r-', label='Data6')
            self.ax4.legend()
            self.ax4.set_title('Data6')
        self.canvas.draw()
    ...
    def create_datashow_frame(self):
        ...
        data1_label = ttk.Label(datashow_frame, text="数据1:", font=('微软雅黑', 12))
        data1_label.grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        self.data1_text = tk.Entry(datashow_frame, width=10, textvariable=self.data1, justify=tk.RIGHT,
                                   font=('Times New Roman', 12))
        self.data1_text.grid(row=0, column=1)

        # 数据2显示
        self.data2 = tk.IntVar()
        data2_label = ttk.Label(datashow_frame, text="数据2:", font=('微软雅黑', 12))
        data2_label.grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        self.data2entry = tk.Entry(datashow_frame, width=10, textvariable=self.data2, justify=tk.RIGHT,
                                   font=('Times New Roman', 12))
        self.data2entry.grid(row=1, column=1)

        # 数据3显示
        self.data3 = tk.IntVar()
        data3_label = ttk.Label(datashow_frame, text="数据3:", font=('微软雅黑', 12))
        data3_label.grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        self.data3entry = tk.Entry(datashow_frame, width=10, textvariable=self.data3, justify=tk.RIGHT,
                                   font=('Times New Roman', 12))  # 高度设置为1行
        self.data3entry.grid(row=2, column=1)

        # 数据4显示
        self.data4 = tk.IntVar()
        data4_label = ttk.Label(datashow_frame, text="数据4:", font=('微软雅黑', 12))
        data4_label.grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
        self.data4entry = tk.Entry(datashow_frame, width=10, textvariable=self.data4, justify=tk.RIGHT,
                                   font=('Times New Roman', 12))  # 高度设置为1行
        self.data4entry.grid(row=3, column=1)

        # 数据5显示
        self.data5 = tk.IntVar()
        data5_label = ttk.Label(datashow_frame, text="数据5:", font=('微软雅黑', 12))
        data5_label.grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        self.data5entry = tk.Entry(datashow_frame, width=10, textvariable=self.data5, justify=tk.RIGHT,
                                   font=('Times New Roman', 12))  # 高度设置为1行
        self.data5entry.grid(row=4, column=1)

        # 数据6显示
        self.data6 = tk.IntVar()
        data6_label = ttk.Label(datashow_frame, text="数据6:", font=('微软雅黑', 12))
        data6_label.grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        self.data6_entry = tk.Entry(datashow_frame, width=10, textvariable=self.data6, justify=tk.RIGHT,
                                    font=('Times New Roman', 12))  # 高度设置为1行
        self.data6_entry.grid(row=5, column=1)
    ...
    def create_monitor_frame(self):
        ...
        self.comboBox1 = ttk.Combobox(monitor_frame,
                                      values=['None', "Data1", "Data2", "Data3", "Data4", "Data5", "Data6"],
                                      style="TCombobox",
                                      width=6)
        self.comboBox2 = ttk.Combobox(monitor_frame,
                                      values=['None', "Data1", "Data2", "Data3", "Data4", "Data5", "Data6"],
                                      style="TCombobox",
                                      width=6)
        # 创建两个下拉框控件
        self.comboBox3 = ttk.Combobox(monitor_frame,
                                      values=['None', "Data1", "Data2", "Data3", "Data4", "Data5", "Data6"],
                                      style="TCombobox",
                                      width=6)
        self.comboBox4 = ttk.Combobox(monitor_frame,
                                      values=['None', "Data1", "Data2", "Data3", "Data4", "Data5", "Data6"],
                                      style="TCombobox",
                                      width=6)
        ...
        # 在界面上将下拉框放置到合适的位置
        ttk.Label(monitor_frame, text="图1", font=('微软雅黑', 12)).grid(row=1, column=0,
                                                                         sticky=tk.W, padx=5)
        self.comboBox1.grid(row=1, column=0, sticky=tk.W, padx=40, pady=5)
        ttk.Label(monitor_frame, text="图2", font=('微软雅黑', 12)).grid(row=1, column=0,
                                                                         sticky=tk.W, padx=115)
        self.comboBox2.grid(row=1, column=0, sticky=tk.W, padx=150, pady=5)
        ttk.Label(monitor_frame, text="图3", font=('微软雅黑', 12)).grid(row=1, column=0,
                                                                         sticky=tk.W, padx=225)
        self.comboBox3.grid(row=1, column=0, sticky=tk.W, padx=260, pady=5)
        ttk.Label(monitor_frame, text="图4", font=('微软雅黑', 12)).grid(row=1, column=0,
                                                                         sticky=tk.W, padx=335)
        ...
        # 绘制数据曲线并设置标签和样式
        self.line1, = self.ax1.plot(self.xdata, self.data1_list, 'r-', label='Data1')
        self.line2, = self.ax2.plot(self.xdata, self.data2_list, 'b-', label='Data2')
        self.line3, = self.ax3.plot(self.xdata, self.data3_list, 'g-', label='Data3')
        self.line4, = self.ax4.plot(self.xdata, self.data4_list, 'c-', label='Data4')
        ...
        self.canvas.get_tk_widget().config(width=800, height=600)  # 设置画布尺寸
        self.canvas.draw()
        self.canvas.get_tk_widget().config(bg='yellow')
        self.canvas.get_tk_widget().grid(row=2, column=0, columnspan=1, sticky=tk.W + tk.S, padx=15, pady=5)
